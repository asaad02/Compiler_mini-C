...
public class Token {
  ...
  public enum Category {
    ...
    // for P5 we add  new tokens CLASS, EXTENDS, and NEW to tokenize the class, extends, and new
    // keywords
    CLASS, // 'class'
    EXTENDS, // 'extends'
    NEW, // 'new'
    ...
    // UpperCaseAlpha = 'A'|...|'Z'
    // Digit          = '0' | '1' | ... | '9'
    IDENTIFIER, // (LowerCaseAlpha | UpperCaseAlpha | '_') (Digit | LowerCaseAlpha | UpperCaseAlpha
    // | '_')*
    ...
    // EscapedChar                   = '\a' | '\b' | '\n' | '\r' | '\t' | '\\' | '\'' | '\"' | '\0'
    // literals
    CHAR_LITERAL, // ''' (LowerCaseAlpha | UpperCaseAlpha | Digit |  SpecialCharWithoutSingleQuote
    // | WhiteSpace | EscapedChar) '''  any character (except single quote) enclosed
    // within  a pair of single quotes
    STRING_LITERAL, // '"' (LowerCaseAlpha | UpperCaseAlpha | Digit |  SpecialCharWithoutDoubleQuote
    //  | WhiteSpace | EscapedChar)* '"'  any sequence of characters (except double
    // quote) enclosed within two double quotes
    INT_LITERAL, // Digit+
  ...
  public String toString() {
    if (data.equals("")) return category.toString();
    else return category.toString() + "(" + data + ")";
  }
}
